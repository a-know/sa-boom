...
$(function(){
	$('#firstTab').click(function(){
		...
		if(!firstLoaded){
			...
			$('#graph_TotalPlayCountRanking_Artist_first').empty();
			$('#graph_TotalPlayCountRanking_Artist_first').css('display', 'none');
			...
			$.ajax({
				...
				success : function(json) {
					...
					$('#graph_TotalPlayCountRanking_Artist_first').css('display', '');
					firstLoaded = true;
...
$(function(){
	$('#secondTab').click(function(){
		...
		if(!secondLoaded){
			//総再生回数ランキング・最新月（アーティスト）用のグラフ生成
			$('#graph_TotalPlayCountRanking_Artist_second').empty();
			$('#graph_TotalPlayCountRanking_Artist_second').css('display', 'none');
			...
			$.ajax({
				...
				success : function(json) {
					...
					$('#graph_TotalPlayCountRanking_Artist_second').css('display', '');
					secondLoaded = true;
...
$(function(){
	$('#thirdTab').click(function(){
		...
		if(!thirdLoaded){
			//総再生回数ランキング・最新月（アーティスト）用のグラフ生成
			$('#graph_TotalPlayCountRanking_Artist_third').empty();
			$('#graph_TotalPlayCountRanking_Artist_third').css('display', 'none');

			var controll_data_artist_graph = {};
			controll_data_artist_graph.functionCode = '9';
			...
			$.ajax({
				...
				success : function(json) {
					...
					$('#graph_TotalPlayCountRanking_Artist_third').css('display', '');
					thirdLoaded = true;

					$.jqplot(
							'graph_TotalPlayCountRanking_Artist_third',[data_playcount_artist],
							{
								title: "総再生回数ランキング＜アーティスト別＞",
								seriesColors: [ "#F5CBCB", "#F2BABA", "#EEA7A7", "#EB9595", "#E88282", "#E57171", "#E15E5E", "#DE4D4D", "#DB3A3A", "#d82727"],
								seriesDefaults: {
		...
		$('#graph_TotalPlayCountRanking_Artist_third').bind('jqplotDataClick',
				function (ev, seriesIndex, pointIndex, data) {
			location.href="/artistSearch?artistName=" + loginIDlist_artist[pointIndex];
		}
		);
